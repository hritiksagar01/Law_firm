...
@section('title', 'Add Team Member — ' . config('legal.app_name', 'Lawyer Workspace'))
...
<div class="max-w-4xl mx-auto p-6 md:p-8">
    ...
    <div class="mb-6">
        ...
        <h1 class="text-2xl font-bold text-[#1a1a1a] tracking-tight">Add Team Member</h1>
        <p class="text-xs text-[#646864] mt-0.5">Onboard an advocate, legal researcher, paralegal, or chambers staff member</p>
    </div>
    ...
    <form method="POST" action="{{ route('users.store') }}" class="space-y-6">
        ...
        <div class="bg-white p-6 rounded-xl border border-[#e5e3dc] shadow-sm">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name') }}" required placeholder="e.g. Adv. Priya Sharma"
                ...
                <div>
                    ...
                    <input type="email" name="email" value="{{ old('email') }}" required placeholder="priya@firm.com"
                        class="w-full px-3.5 py-2 text-xs rounded-lg border border-[#e5e3dc] bg-[#faf8f5] focus:bg-white focus:outline-none focus:border-[#23493a] focus:ring-1 focus:ring-[#23493a] text-[#1a1a1a]" />
                </div>

                <div>
                    <label class="block text-xs font-medium text-[#1a1a1a] mb-1.5">Professional Title / Designation</label>
                    <input type="text" name="title" value="{{ old('title') }}" placeholder="e.g. Senior Associate, Criminal Defense"
                        class="w-full px-3.5 py-2 text-xs rounded-lg border border-[#e5e3dc] bg-[#faf8f5] focus:bg-white focus:outline-none focus:border-[#23493a] focus:ring-1 focus:ring-[#23493a] text-[#1a1a1a]" />
                </div>
                ...
                <div class="md:col-span-2">
                    <label class="block text-xs font-medium text-[#1a1a1a] mb-1.5">Temporary Initial Password <span class="text-rose-500">*</span></label>
        ...
        <div class="bg-white p-6 rounded-xl border border-[#e5e3dc] shadow-sm">
            ...
            <div class="space-y-4">
                <div>
                    <label class="block text-xs font-medium text-[#1a1a1a] mb-2">Select Practice Role <span class="text-rose-500">*</span></label>
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        ...
                        <label class="flex items-start gap-3 p-3 rounded-lg border border-[#e5e3dc] bg-[#faf8f5] hover:bg-white cursor-pointer transition-colors">
                            <input type="radio" name="role_id" value="{{ $r->id }}" {{ old('role_id') == $r->id || (old('role_id') == null && $r->slug === 'lawyer') ? 'checked' : '' }}
                ...
                <div class="pt-4 border-t border-[#e5e3dc]">
                    <label class="block text-xs font-medium text-[#1a1a1a] mb-1">Assign Practice Groups</label>
                    <p class="text-[11px] text-[#646864] mb-2.5">Member will collaborate with other advocates in selected groups</p>
                    <div class="grid grid-cols-2 sm:grid-cols-3 gap-2.5">
                        ...
                        <label class="flex items-center gap-2 p-2.5 rounded-lg border border-[#e5e3dc] bg-[#faf8f5] hover:bg-white cursor-pointer text-xs transition-colors">
                            <input type="checkbox" name="groups[]" value="{{ $g->id }}"
                                {{ is_array(old('groups')) && in_array($g->id, old('groups')) ? 'checked' : '' }}
