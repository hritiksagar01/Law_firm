...
@section('title', 'Draft Legal Opinion — ' . config('legal.app_name', 'Vennamraj Associates'))

@section('content')
<div class="max-w-4xl mx-auto p-6 md:p-8">

    <div class="mb-8">
        <a href="{{ route('opinions.index') }}" class="inline-flex items-center gap-1 text-xs text-[#766A5E] hover:text-[#9F8349] mb-2">
            <span class="material-symbols-outlined text-sm">arrow_back</span>
            <span>Back to Legal Opinions</span>
        </a>
        <h1 class="text-3xl font-serif font-bold text-[#222222]">Draft Legal Opinion</h1>
        <p class="text-sm text-[#766A5E] mt-1">Author formal legal advice, litigation strategy memo, or document review advisory</p>
    </div>
    ...
    <form method="POST" action="{{ route('opinions.store') }}" class="space-y-6">
        ...
        <div class="bg-white p-6 rounded-2xl border border-[#EFECE6] shadow-xs">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    <label class="block text-xs font-semibold text-[#222222] mb-1">Associated Case / Matter (Optional)</label>
                ...
                <div>
                    <label class="block text-xs font-semibold text-[#222222] mb-1">Client (Optional)</label>
        ...
        <div class="bg-white p-6 rounded-2xl border border-[#EFECE6] shadow-xs space-y-4">
            ...
            <div>
                ...
                <textarea name="summary" rows="3" placeholder="Succinct background summary of facts and legal questions posed..."
                    class="w-full px-3.5 py-2 text-xs rounded-lg border border-[#EAE4DC] bg-[#FAF8F5] focus:outline-none focus:border-[#9F8349]">{{ old('summary') }}</textarea>
            </div>
            ...
            <div>
                ...
                <textarea name="body" rows="10" required placeholder="Detailed analysis, statutory interpretations, arguments on merits, and legal reasoning..."
                    class="w-full px-3.5 py-2 text-xs font-sans rounded-lg border border-[#EAE4DC] bg-[#FAF8F5] focus:outline-none focus:border-[#9F8349] leading-relaxed">{{ old('body') }}</textarea>
            </div>
            ...
            <div>
                ...
                <textarea name="recommendations" rows="3" placeholder="Actionable steps for the client or trial team (e.g. file writ petition, seek interim relief)..."
                    class="w-full px-3.5 py-2 text-xs rounded-lg border border-[#EAE4DC] bg-[#FAF8F5] focus:outline-none focus:border-[#9F8349]">{{ old('recommendations') }}</textarea>
            </div>
            ...
            <div>
                ...
                <textarea name="precedents_cited" rows="3" placeholder="e.g. (1) AIR 2021 SC 1450, (2) Section 11 of Civil Procedure Code..."
                    class="w-full px-3.5 py-2 text-xs font-mono rounded-lg border border-[#EAE4DC] bg-[#FAF8F5] focus:outline-none focus:border-[#9F8349]">{{ old('precedents_cited') }}</textarea>
            </div>
        </div>
        ...
        <div class="bg-white p-6 rounded-2xl border border-[#EFECE6] shadow-xs flex items-center justify-between">
            <div>
                <label class="block text-xs font-semibold text-[#222222]">Initial Workflow Stage</label>
                <p class="text-[11px] text-[#766A5E]">Draft opinions can be edited; Published opinions are formal records</p>
            </div>
        ...
        <div class="flex items-center justify-end gap-3 pt-2">
            <a href="{{ route('opinions.index') }}" class="px-5 py-2.5 rounded-lg border border-[#EAE4DC] bg-white text-[#222222] text-xs font-semibold hover:bg-[#FAF8F5]">
                Cancel
            </a>
