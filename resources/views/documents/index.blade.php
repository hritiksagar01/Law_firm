    ...
    <div x-data="{ openUploadModal: false, categoryFilter: 'all' }">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-[#EFECE6] mb-6">
            ...
            <div class="flex items-center gap-2">
                <button @click="openUploadModal = true" class="inline-flex items-center gap-2 px-4 h-10 rounded-lg bg-[#845D33] text-white text-xs font-medium hover:bg-[#6D4B27] shadow-sm transition-colors">
        ...
        <div @click="openUploadModal = true" class="p-8 mb-6 rounded-xl border-2 border-dashed border-[#EAE4DC] bg-white hover:border-[#845D33] transition-colors flex flex-col items-center justify-center text-center cursor-pointer group">
            <div class="w-12 h-12 rounded-full bg-[#FAF8F5] group-hover:bg-[#F4ECE1]/50 flex items-center justify-center text-[#845D33] mb-3 transition-colors">
            ...
            <span class="mt-3 inline-flex items-center px-3 py-1 rounded bg-[#F4EFEA] text-[#554D45] text-xs font-medium group-hover:bg-[#845D33] group-hover:text-white transition-colors">Select Document File</span>
        </div>
        ...
        <div class="bg-white rounded-xl border border-[#EFECE6] shadow-sm overflow-hidden">
            <div class="p-4 border-b border-[#F4EFEA] flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div class="flex items-center gap-2">
                    <span class="material-symbols-outlined text-[#845D33] text-xl">folder_special</span>
                ...
                <div class="flex items-center gap-1 bg-[#FAF8F5] p-1 rounded-lg border border-[#EFECE6] text-xs">
                    <button @click="categoryFilter = 'all'" :class="categoryFilter === 'all' ? 'bg-white font-semibold text-[#845D33] shadow-xs' : 'text-[#766A5E]'" class="px-2.5 py-1 rounded">All</button>
                    <button @click="categoryFilter = 'Pleading'" :class="categoryFilter === 'Pleading' ? 'bg-white font-semibold text-[#845D33] shadow-xs' : 'text-[#766A5E]'" class="px-2.5 py-1 rounded">Pleadings</button>
                    <button @click="categoryFilter = 'Motion'" :class="categoryFilter === 'Motion' ? 'bg-white font-semibold text-[#845D33] shadow-xs' : 'text-[#766A5E]'" class="px-2.5 py-1 rounded">Motions</button>
                    <button @click="categoryFilter = 'Exhibit'" :class="categoryFilter === 'Exhibit' ? 'bg-white font-semibold text-[#845D33] shadow-xs' : 'text-[#766A5E]'" class="px-2.5 py-1 rounded">Exhibits</button>
                    <button @click="categoryFilter = 'Deposition'" :class="categoryFilter === 'Deposition' ? 'bg-white font-semibold text-[#845D33] shadow-xs' : 'text-[#766A5E]'" class="px-2.5 py-1 rounded">Depositions</button>
                </div>
            </div>
            ...
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y divide-[#F4EFEA] text-sm">
                    ...
                    <tr x-show="categoryFilter === 'all' || categoryFilter === '{{ $doc->category }}'" class="hover:bg-[#FAF8F5] transition-colors group">
                        <td class="py-3.5 px-4">
                            <div class="flex items-center gap-3">
                                ...
                                <div class="flex flex-col">
                                    <span class="font-semibold text-xs text-[#222222] group-hover:text-[#845D33] transition-colors">{{ $doc->title }}</span>
                        ...
                        <td class="py-3.5 px-4">
                            <span class="font-mono text-[10px] px-2 py-0.5 rounded bg-[#B88B56]/60 text-[#845D33] font-semibold">
                                {{ $doc->privilege }}
                            </span>
                        </td>
                        ...
                        <td class="py-3.5 px-4 text-center">
                            <a href="{{ route('documents.download', $doc->id) }}" class="p-1.5 text-[#766A5E] hover:text-[#845D33] hover:bg-[#F4EFEA] rounded inline-flex" title="Download Verified File">
        ...
        <div x-show="openUploadModal" @click.away="openUploadModal = false" x-transition class="fixed inset-0 bg-black/40 backdrop-blur-xs z-50 flex items-center justify-center p-4">
            <div class="bg-white rounded-xl shadow-2xl border border-[#EFECE6] w-full max-w-lg p-6 flex flex-col">
                <div class="flex items-center justify-between pb-4 border-b border-[#F4EFEA] mb-4">
                    <div class="flex items-center gap-2">
                        <span class="material-symbols-outlined text-[#845D33]">upload_file</span>
                ...
                <form action="{{ route('documents.upload') }}" method="POST" enctype="multipart/form-data" class="flex flex-col gap-3.5 text-xs">
                    ...
                    <div class="flex flex-col gap-1">
                        ...
                        <input name="title" required type="text" placeholder="e.g. Plaintiff's Response to Defendant's Motion to Dismiss" class="w-full px-3 py-2 rounded-lg bg-white border border-[#EAE4DC] outline-none focus:border-[#845D33]"/>
                    </div>

                    <div class="grid grid-cols-2 gap-3">
                        <div class="flex flex-col gap-1">
                            <label class="font-medium text-[#222222]">Filing Category</label>
                            <select name="category" class="w-full px-3 py-2 rounded-lg bg-white border border-[#EAE4DC] outline-none focus:border-[#845D33]">
                        ...
                        <div class="flex flex-col gap-1">
                            ...
                            <select name="privilege" class="w-full px-3 py-2 rounded-lg bg-white border border-[#EAE4DC] outline-none focus:border-[#845D33]">
                    ...
                    <div class="flex flex-col gap-1">
                        ...
                        <input name="file" required type="file" class="w-full p-2 rounded-lg bg-[#FAF8F5] border border-[#EAE4DC] file:mr-3 file:py-1 file:px-3 file:rounded file:border-0 file:text-xs file:font-medium file:bg-[#845D33] file:text-white hover:file:bg-[#6D4B27] cursor-pointer"/>
                    </div>

                    <div class="p-2.5 rounded bg-[#F8F4EE] text-[#845D33] text-[11px] flex items-center gap-1.5">
                    ...
                    <div class="pt-3 border-t border-[#F4EFEA] flex items-center justify-end gap-2">
                        ...
                        <button type="submit" class="px-4 py-2 rounded-lg bg-[#845D33] text-white font-semibold hover:bg-[#6D4B27]">Authenticate &amp; Upload</button>
                    </div>
                </form>
            </div>
        </div>
