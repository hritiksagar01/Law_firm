    ...
    <div x-data="{ openInvoiceModal: false }">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-[#EFECE6] mb-6">
            ...
            <div class="flex items-center gap-2">
                <button @click="openInvoiceModal = true" class="inline-flex items-center gap-2 px-4 h-10 rounded-lg bg-[#9F8349] text-white text-xs font-medium hover:bg-[#856C36] shadow-sm transition-colors">
        ...
        <div class="p-6 mb-6 rounded-xl border border-[#F4ECE1] bg-[#F8F4EE] shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div class="flex items-start gap-4">
                <div class="w-12 h-12 rounded-full bg-[#9F8349] text-white flex items-center justify-center shrink-0">
                    <span class="material-symbols-outlined text-2xl">verified_user</span>
                </div>
                <div>
                    <span class="font-mono text-[10px] font-bold uppercase tracking-wider text-[#856C36]">Fiduciary Client Trust &amp; Advance Audit</span>
                    <h3 class="text-lg font-serif font-bold text-[#222222]">Advance Retainer Trust Ledger Verified</h3>
                    <p class="text-xs text-[#9F8349] mt-0.5">Bank Statement Balance = Individual Client Retainer Sub-Ledgers (Bar Council Standard)</p>
                </div>
            </div>

            <div class="flex items-center gap-6 divide-x divide-[#F4ECE1]">
                <div class="flex flex-col">
                    <span class="text-[10px] font-mono uppercase text-[#9F8349]">Total Client Advance Held</span>
                ...
                <div class="pl-6 flex flex-col">
                    <span class="text-[10px] font-mono uppercase text-[#9F8349]">Audit Variance</span>
                    <span class="font-mono text-xl font-bold text-[#9F8349]">{{ config('legal.currency_symbol', '₹') }}0.00</span>
                </div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 xl:grid-cols-2 gap-6">
            ...
            <div class="bg-white rounded-xl border border-[#EFECE6] shadow-sm overflow-hidden">
                <div class="p-4 border-b border-[#F4EFEA] flex items-center justify-between">
                    <div class="flex items-center gap-2">
                        <span class="material-symbols-outlined text-[#9F8349] text-xl">receipt</span>
                        ...
                        <tr class="hover:bg-[#FAF8F5]">
                            ...
                            <td class="py-3 px-4 text-center">
                                <div class="flex items-center justify-center gap-1.5">
                                    <span class="font-mono text-[10px] px-2 py-0.5 rounded font-bold uppercase {{ $inv->status === 'paid' ? 'bg-[#F4ECE1] text-[#856C36]' : 'bg-[#B88B56]/60 text-[#9F8349]' }}">
                                        {{ $inv->status }}
                                    </span>
                                    @if($inv->status !== 'paid')
                                    <form action="{{ route('invoices.pay', $inv->id) }}" method="POST" class="inline">
                                        @csrf
                                        <button type="submit" class="font-mono text-[9px] px-1.5 py-0.5 rounded bg-[#F8F4EE] hover:bg-[#F4ECE1] text-[#856C36] border border-[#D4C4B5] transition-colors" title="Mark Settled via Client Advance or Direct Transfer">
                                            Settle
                                        </button>
                                    </form>
                                    @endif
            ...
            <div class="bg-white rounded-xl border border-[#EFECE6] shadow-sm overflow-hidden">
                <div class="p-4 border-b border-[#F4EFEA] flex items-center justify-between">
                    <div class="flex items-center gap-2">
                        <span class="material-symbols-outlined text-[#9F8349] text-xl">timelapse</span>
                        <h3 class="text-sm font-semibold text-[#222222]">Unbilled Professional Hours</h3>
                    </div>
                    <span class="font-mono text-xs font-bold text-[#9F8349]">{{ config('legal.currency_symbol', '₹') }}{{ number_format(\App\Models\TimeEntry::where('status', 'unbilled')->sum('total_amount'), 2) }}</span>
                </div>
                ...
                <div class="divide-y divide-[#F4EFEA]">
                    ...
                    <div class="p-3.5 flex items-start justify-between gap-3 text-xs hover:bg-[#FAF8F5]">
                        <div class="flex items-start gap-2.5">
                            <span class="font-mono font-bold text-[#9F8349] w-10 shrink-0">{{ $te->hours }}h</span>
                            <div class="flex flex-col">
                        ...
                        <div class="flex flex-col items-end">
                            ...
                            <span class="font-mono text-[9px] uppercase px-1.5 rounded {{ $te->status === 'unbilled' ? 'bg-amber-100 text-amber-800' : 'bg-[#F4ECE1] text-[#856C36]' }}">{{ $te->status }}</span>
                        </div>
                    </div>
                    @empty
        ...
        <div x-show="openInvoiceModal" @click.away="openInvoiceModal = false" x-transition class="fixed inset-0 bg-black/40 backdrop-blur-xs z-50 flex items-center justify-center p-4">
            <div class="bg-white rounded-xl shadow-2xl border border-[#EFECE6] w-full max-w-md p-6 flex flex-col">
                <div class="flex items-center justify-between pb-4 border-b border-[#F4EFEA] mb-4">
                    <div class="flex items-center gap-2">
                        <span class="material-symbols-outlined text-[#9F8349]">receipt_long</span>
                ...
                <form action="{{ route('invoices.generate') }}" method="POST" class="flex flex-col gap-3.5 text-xs">
                    ...
                    <div class="p-3 rounded-lg bg-[#F8F4EE] border border-[#F4ECE1] text-xs text-[#9F8349] flex flex-col gap-1">
                    ...
                    <div class="pt-3 border-t border-[#F4EFEA] flex items-center justify-end gap-2">
                        ...
                        <button type="submit" class="px-4 py-2 rounded-lg bg-[#9F8349] text-white font-semibold hover:bg-[#856C36]">Generate &amp; Dispatch</button>
                    </div>
                </form>
            </div>
        </div>
