...
<head>
    ...
    <link
        href="https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700&family=Newsreader:ital,opsz,wght@0,6..72,400;0,6..72,500;0,6..72,600;1,6..72,400&family=Material+Symbols+Outlined:opsz,wght,FILL,GRAD@20..48,100..700,0..1,-50..200&display=swap"
        rel="stylesheet" />
...
<body
    ...
    <div class="w-full max-w-[480px] flex flex-col items-center my-6">
        ...
        <div class="flex flex-col items-center text-center mb-6">
            <div
                class="p-4 bg-white rounded-2xl border border-[#E7E4DC] shadow-[0_4px_24px_-2px_rgba(26,30,28,0.08)] flex items-center justify-center">
                <img src="{{ \App\Models\PlatformSetting::logoUrl() }}"
                    alt="{{ \App\Models\PlatformSetting::platformName() }}"
        ...
        @if(session('success'))
            <div
                class="w-full mb-4 p-3 rounded-[6px] bg-[#23493A]/10 border border-[#23493A]/20 text-xs text-[#23493A] flex items-center gap-2">
        ...
        @if(session('info'))
            <div
                class="w-full mb-4 p-3 rounded-[6px] bg-[#F6F4EE] border border-[#E7E4DC] text-xs text-[#1A1E1C] flex items-center gap-2">
        ...
        @if(session('error'))
            <div
                class="w-full mb-4 p-3 rounded-[6px] bg-red-50 border border-red-200 text-xs text-red-700 flex items-center gap-2">
        ...
        <div
            class="w-full bg-white rounded-xl border border-[#E7E4DC] shadow-[0_4px_24px_-2px_rgba(26,30,28,0.06),0_0_0_1px_#E7E4DC] p-6 sm:p-7 flex flex-col relative">
            ...
            <div class="mb-5">
                ...
                <div class="grid grid-cols-2 p-1 bg-[#F6F4EE] rounded-lg border border-[#E7E4DC] gap-1 mb-2.5">
                    ...
                    <button type="button" @click="setCredential('client', 'hritik.srivastava28@gmail.com', '12345678')"
                        ...
                        <span
                            class="w-3.5 h-3.5 rounded-full border flex items-center justify-center transition-all shrink-0"
            ...
            <div class="mb-5 pb-3 border-b border-[#F0EEE8]">
                <template x-if="tab === 'client'">
                    <div>
                        <h2 class="font-headline text-[22px] font-medium text-[#1A1E1C] tracking-tight">Client Portal
                        </h2>
                        <p class="text-xs text-[#646864] mt-0.5 font-sans">Direct access for enterprise retainers, case
                            filings, and legal opinions.</p>
                    </div>
                </template>
                ...
                <template x-if="tab === 'admin'">
                    <div>
                        <h2 class="font-headline text-[22px] font-medium text-[#1A1E1C] tracking-tight">Super Admin
                            Portal</h2>
                        <p class="text-xs text-[#646864] mt-0.5 font-sans">Super-administrator console for multi-tenant
                            firm governance.</p>
                    </div>
                </template>
            </div>
            ...
            <form action="{{ route('login') }}" method="POST" class="flex flex-col gap-4">
                ...
                <div class="flex flex-col gap-1.5">
                    ...
                    <div class="relative flex items-center">
                        <span
                            class="material-symbols-outlined absolute left-3 text-[#8A8E89] text-[18px] pointer-events-none">mail</span>
                ...
                <div class="flex flex-col gap-2">
                    ...
                    <div class="flex items-center justify-between gap-1.5 sm:gap-2" @paste="handlePaste($event)">
                        <template x-for="(digit, index) in passcode" :key="index">
                            <input :type="showPasscode ? 'text' : 'password'" maxlength="1" inputmode="numeric"
                                :x-ref="'box' + index" :value="passcode[index]" @input="handleInput($event, index)"
                    ...
                    <div class="flex items-center justify-between mt-1">
                        <button type="button" @click="showPasscode = !showPasscode"
                            ...
                            <span class="material-symbols-outlined text-[15px]"
                                x-text="showPasscode ? 'visibility_off' : 'visibility'">visibility</span>
                ...
                <button type="submit"
                    ...
                    <span
                        x-text="tab === 'client' ? 'Sign In to Client Portal' : (tab === 'firm' ? 'Sign In to Law Practice' : 'Access Super Admin')">Sign
                        In</span>
            ...
            <div class="mt-4 pt-3 border-t border-[#E7E4DC] flex flex-col gap-2">
                <span class="text-[11px] font-medium text-[#8A8E89] uppercase tracking-wider">Quick Fill Demo
                    Accounts</span>
    ...
    <footer class="w-full max-w-[500px] text-center my-4 text-[12px] space-y-1.5">
        <div class="flex items-center justify-center gap-3 text-[11.5px] text-[#646864]">
            <a href="{{ route('public.about') }}" target="_blank"
                class="hover:text-[#23493A] transition-colors">About</a>
            <span>&middot;</span>
            <a href="{{ route('public.contact') }}" target="_blank"
                class="hover:text-[#23493A] transition-colors">Contact Registry</a>
            <span>&middot;</span>
            <a href="{{ route('public.privacy') }}" target="_blank"
                class="hover:text-[#23493A] transition-colors">Privilege &amp; Privacy</a>
            <span>&middot;</span>
            <a href="{{ route('public.terms') }}" target="_blank"
                class="hover:text-[#23493A] transition-colors">Terms</a>
        </div>
