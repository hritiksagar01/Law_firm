...
<body class="bg-[#FAF8F5] text-[#222222] font-sans antialiased min-h-screen flex flex-col justify-center items-center p-4 relative" x-data="{ tab: 'firm', showPassword: false }">
    ...
    <div class="w-full max-w-md flex flex-col items-center my-8">
        ...
        <div class="flex flex-col items-center text-center mb-6">
            <img src="{{ asset('logo.png') }}" alt="{{ config('legal.app_name', 'Vennamraj Associates') }}" class="h-20 w-auto object-contain rounded-xl shadow-md border border-[#E8DAC8]/60 bg-white p-2"/>
        </div>

        <!-- Flash Notifications -->
        @if(session('success'))
        <div class="w-full mb-4 p-3 rounded-lg bg-emerald-50 border border-emerald-200 text-xs text-emerald-800 flex items-center gap-2 shadow-xs">
        ...
        @if(session('info'))
        <div class="w-full mb-4 p-3 rounded-lg bg-[#F8F4EE] border border-[#9F8349]/30 text-xs text-[#856C36] flex items-center gap-2 shadow-xs">
        ...
        @if(session('error'))
        <div class="w-full mb-4 p-3 rounded-lg bg-red-50 border border-red-200 text-xs text-red-800 flex items-center gap-2 shadow-xs">
        ...
        <div class="w-full bg-white rounded-2xl shadow-xl border border-[#EFECE6] p-7 flex flex-col relative">
            ...
            <div class="mb-5">
                <template x-if="tab === 'firm'">
                    <div>
                        ...
                        <p class="text-xs text-[#766A5E] mt-0.5">Enter your practice credentials to access litigation dockets, case files, and opinions.</p>
                    </div>
                </template>
                <template x-if="tab === 'client'">
                    <div>
                        <h2 class="text-base font-serif font-bold text-[#222222]">Client Portal Sign In</h2>
                        <p class="text-xs text-[#766A5E] mt-0.5">Enter your invited client credentials to track case progress, review filings, and message counsel.</p>
                    </div>
                </template>
            </div>
            ...
            <form action="{{ route('login') }}" method="POST" class="flex flex-col gap-4">
                ...
                <div class="flex flex-col gap-1.5">
                    ...
                    <div class="relative flex items-center">
                        ...
                        <input name="email" value="{{ old('email') }}" type="email" required autofocus placeholder="name@domain.com" class="w-full pl-10 pr-3 py-2.5 rounded-lg bg-[#FAF8F5] border border-[#EAE4DC] text-xs text-[#222222] outline-none focus:bg-white focus:border-[#9F8349] transition-all"/>
                    </div>
                </div>
                ...
                <div class="flex flex-col gap-1.5">
                    ...
                    <div class="relative flex items-center">
                        ...
                        <input name="password" :type="showPassword ? 'text' : 'password'" required placeholder="Enter your password" class="w-full pl-10 pr-10 py-2.5 rounded-lg bg-[#FAF8F5] border border-[#EAE4DC] text-xs text-[#222222] outline-none focus:bg-white focus:border-[#9F8349] transition-all"/>
                        <button type="button" @click="showPassword = !showPassword" class="absolute right-3 text-[#766A5E] hover:text-[#222222] transition-colors cursor-pointer">
                ...
                <div class="flex items-center justify-between py-1">
                    <label class="flex items-center gap-2 cursor-pointer select-none">
                        <input type="checkbox" name="remember" value="1" class="w-4 h-4 rounded text-[#9F8349] accent-[#9F8349] cursor-pointer"/>
                        <span class="text-xs text-[#554D45]">Remember my session</span>
                    </label>
                </div>

                <button type="submit" class="w-full py-2.5 px-4 rounded-lg bg-[#9F8349] text-white text-xs font-semibold flex items-center justify-center gap-2 hover:bg-[#856C36] shadow-sm transition-all active:scale-[0.99] mt-1 cursor-pointer">
            ...
            <div class="mt-6 pt-5 border-t border-[#EFECE6] flex items-center justify-between">
                ...
                <a href="{{ route('register') }}" class="py-1.5 px-3 rounded-lg bg-[#FAF8F5] border border-[#EAE4DC] text-[#9F8349] text-[11px] font-semibold hover:bg-white hover:border-[#9F8349] transition-colors shrink-0">
                    Register Practice
                </a>
            </div>
